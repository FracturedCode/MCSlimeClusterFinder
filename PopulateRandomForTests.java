...
public class PopulateRandomForTests {
    public static void main(String[] args) {
        new PopulateRandomForTests();
        long worldseed = 423338365327502521L;
        int x = -12500;
        int z = -12489;
        long seed = (worldseed + (x * x * 4987142) + (x * 5947611) + (z * z * 4392871) + ((z * 389711) ^ 987234911L));
        System.out.println(worldseed + "\n" + (x * x * 4987142) + "\n" + (x * 5947611) + "\n" + (z * z * 4392871) + "\n" + ((z * 389711) ^ 987234911L) + "\n" + seed);
        System.out.println(new java.util.Random(seed).nextInt(10));
    }

    public PopulateRandomForTests() {
        int iter = 10000;
        Result[] results = new Result[iter];
        for (int i = 0; i<iter; i++) {
            Result r = new Result();
            Random rng = new Random(i);
            r.inputSeed = i;
            r.initSeed = rng.seed;
            r.nextInt = rng.nextInt(10);
            r.finalSeed = rng.seed;
            results[i] = r;
        }

        try {
            File file = new File("randomOutput.txt");
            file.createNewFile();
            FileWriter fr = new FileWriter("randomOutput.txt");
            for (Result r : results) {
                fr.write(r.inputSeed + "," + r.initSeed + "," + r.nextInt + "," + r.finalSeed + "\n");
            }
            fr.close();
        } catch (IOException e) {
            e.printStackTrace();
